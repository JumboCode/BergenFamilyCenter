...
export default function WeeklyCalendar({ selectedDay, user, setSelectedDay }) {
    ...
    useEffect(() => {
        firebaseFilterEventsChronologicalWeek(startOfWeek, endOfWeek, divisions).then(v => {
            const eventDays = [];
            const overlaps = [];
            for (let i = 0; i < 7; i++) {
                eventDays.push([]);
                ...
                eventDays[day].forEach((e, z) => {
                    if (e !== event) {
                        if (eventsOverlap(e, event) && dayIndex > z) {
                            if (start.getTime() > roundToNearest15(e.startTime.toDate()).getTime()) {
                                overlap = true;
                                overlapAmount += (overlaps[i - 1] + 1);
                            } else if (start.getTime() === roundToNearest15(e.startTime.toDate()).getTime() && end.getTime() !== roundToNearest15(e.endTime.toDate()).getTime() && !overlap) {
    ...
    return (
        <div>
            <div className='week'>
                <Grid container direction="row" sx={{ height: "100%" }} alignItems="center">
                    <Grid item>
                        <IconButton onClick={() => {
            ...
            <Box className={clsx("schedule-container", classes.root)} sx={{ display: "flex", flexDirection: "column", overflow: "auto" }}>
                {!open ? null :
                    <EventDialog
                        open={open}
                        setOpen={setOpen}
                ...
                <div className="schedule" aria-labelledby="schedule-heading">
                    ...
                    {/* Time */}
                    <span className="track-slot" aria-hidden="true" style={{ gridColumn: "times / track-8", gridRow: "tracks" }} ></span>
                    ...
                    {events.map(e => e)}
                    {times.map((t, i) => (
                        <Typography variant='inherit' key={i} className="time-slot" style={{ gridRow: `time-${mapTime(t[0], t[1])}` }}>{`${t[1] == 0 ? (t[0] > 12 ? t[0] % 12 : t[0]) : ""}`}</Typography>
                    ))}
